                            ...
                            <div class="bg-white card card-user">
                                <div class="card-header d-flex">
                                    <h5 class="card-title">Datos del pedido</h5>
                                        <a  onclick="imprimirPedido();"
                                        class="btn btn-sm btn-warning ml-auto">Imprimir pedido
                                    </a>
                                </div>
                                <div class="card-body">
                                        ...
                                        <table class="table" id="tablaPedido">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Descuento</th>
                                                    <th class="tdMontoTotal">MONTO TOTAL</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                            	   @php
                                            ...
                                            @endphp
                                                ...
                                                @endphp
                                                @endforeach
									<tr class="{{$numerotrproducto}} trsubtotal">
...
function imprimirPedido(){
    ...
    var pdf= new jsPDF({orientation: 'p',
                                        unit: 'cm',
                                        format: 'a4'});

        var tablaParaImprimir=$("#tablaPedido").clone()
            tablaParaImprimir.find('.tdUnidadesAEntregar .input-group').remove()
            tablaParaImprimir.find('.tdKilosAEntregar .input-group').remove()
            tablaParaImprimir.find('.tdMontoTotal').remove()
            tablaParaImprimir.find('.trsubtotal').remove()

        var tablaJson=pdf.autoTableHtmlToJson(tablaParaImprimir.get(0))

        pdf.autoTable(tablaJson.columns, tablaJson.data,{startY:3.5, margin: {top: 3.5, right: 0.5, bottom: 3, left: 0.5},headStyles :{fontSize:7,cellPadding:0.1,halign:'center',valign:'middle',fillColor:[26,40,52]},bodyStyles:{fontSize:9,cellPadding:0.2,halign:'center',valign:'middle',lineWidth:0.01,lineColor:0}});

        var nropedido={{$pedido->id_pedido}};
        var nombrecliente="{{$pedido->vendedor['nombre']}}";
        var nombre="Pedido N° "+nropedido+" para "+nombrecliente+" PURARES Clientes";
...
const pageHeight = pdf.internal.pageSize.height;  //Optional
      ...
      let horizontalPos = pageWidth - 2.5;  //Can be fixed number
      let verticalPos = pageHeight - 27.2;  //Can be fixed number
      pdf.setPage(j);
      pdf.setTextColor('#436784').setFont('Century Gothic').setFontStyle('bold').setFontSize(10).text(`${j} de ${pages}`, horizontalPos, verticalPos)

        pdf.setTextColor('#436784').setFont('Century Gothic').setFontStyle('bold').setFontSize(15).text(6,2,'Pedido N° '+nropedido+' para '+nombrecliente);
      pdf.setFontSize(10).text(17,2.5,"Página: ");
        pdf.setFontSize(10).text(17,1.5,'Impreso '+fecha);

   pdf.addImage(logo,'JPEG',0.9,1.5,4.4795,0.986);

        pdf.setLineWidth(0.1);
        pdf.setDrawColor(26,40,52);
        pdf.line(0.53, 2.9, 20.5, 2.9);
